...
export default class CadastroAlunos extends React.Component {
    ...
    componentDidMount() {
        ...
        if (params.id) {
            ...
            this.alunoService.buscarAlunoPorID(params.id)
                .then(response => {
                    this.setState({ ...response.data })
                    const user = response.data
                    console.log(user)
                    const { endereco: { rua, bairro, numero, cidade } } = user
                    this.setState({rua: rua,numero: numero,bairro: bairro,cidade: cidade})
                    
                    const { contato: { email, telefone } } = user
                    this.setState({email: email,telefone: telefone})
    ...
    save = () => {
        ...
        if (this.state.id == null) {
            ...
            this.alunoService.atualizarAluno(user)
                .then(user => {
                ...
                <div className="container">
                    <div className="row">
                        <div className="col-6" style={{ position: 'relative', left: '300px' }}>
                            <Card titulo="Cadastro de Alunos">
                                <div className="row">
                                                                ...
                                                                <select className="form-control"
                                                                    ...
                                                                    <option></option>
                                                                    {
                                                                        listCursos.map(cursos => (
                                                                            <option key={cursos.id} value={cursos.id}>{cursos.nomeCurso}</option>
                                                                        ))
                                                                    }
                                                                </select>
                                                            </div>
